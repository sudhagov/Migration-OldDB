...
namespace Migration
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            table1.Clear();
            if (!table1.Columns.Contains("Controller"))
            {
                table1.Columns.Add("Controller");
            ...
            table3.Clear();
            if (!table3.Columns.Contains("NodeID"))
            {
                table3.Columns.Add("NodeID");
                table3.Columns.Add("Report Time");
                table3.Columns.Add("Enclosure Temp");
                table3.Columns.Add("Ambient Temp");
                table3.Columns.Add("Enclosure Type");
                table3.Columns.Add("Location Low Temp");
                table3.Columns.Add("DeltaT");
                table3.Columns.Add("Battery Level");
                table3.Columns.Add("DeviceMAC");
                table3.Columns.Add("Sensor Name");
                table3.Columns.Add("Humidity");
            }
            int cnt = 1;

            if (File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86) + "\\DeltaTAlert\\Resources\\ApplicationData.bin"))
            {
                ultraDataSource1.LoadFromBinary(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86) + "\\DeltaTAlert\\Resources\\ApplicationData.bin");
                Infragistics.Win.UltraWinDataSource.UltraDataRow row = ultraDataSource1.Rows[0];
                if (row["Controller1"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller1"], row["Controller1Name"], row["Controller1MAC"]);

                }
                if (row["Controller2"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller2"], row["Controller2Name"], row["Controller2MAC"]);
                }
                if (row["Controller3"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller3"], row["Controller3Name"], row["Controller3MAC"]);
                }
                if (row["Controller4"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller4"], row["Controller4Name"], row["Controller4MAC"]);
                }
                if (row["Controller5"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller5"], row["Controller5Name"], row["Controller5MAC"]);
                }
                if (row["Controller6"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller6"], row["Controller6Name"], row["Controller6MAC"]);
                }
                if (row["Controller7"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller7"], row["Controller7Name"], row["Controller7MAC"]);
                }
                if (row["Controller8"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller8"], row["Controller8Name"], row["Controller8MAC"]);
                }
                if (row["Controller9"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller9"], row["Controller9Name"], row["Controller9MAC"]);
                }
                if (row["Controller10"] != "")
                {
                    table1.Rows.Add(cnt++, row["Controller10"], row["Controller10Name"], row["Controller10MAC"]);
                }
            ...
            if(File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86) + "\\DeltaTAlert\\Resources\\SensorData.bin"))
            {
                ultraDataSource1.LoadFromBinary(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86) + "\\DeltaTAlert\\Resources\\SensorData.bin");
                for(int i=0; i< ultraDataSource1.Rows.Count; i++)
                {
                    Infragistics.Win.UltraWinDataSource.UltraDataRow row = ultraDataSource1.Rows[i];
                    table3.Rows.Add(row["NodeID"], row["Report Time"], row["Enclosure Temp"], row["Ambient Temp"], row["Enclosure Type"], row["Location Low Temp"], row["DeltaT"], row["Battery Level"], row["DeviceMAC"], row["Sensor Name"], row["Humidity"]);
                }
            }
            ultraGrid2.DataSource = table3;
                    ...
                    if (zwFrame.CommandClass == ZWConst.COMMAND_CLASS_ZIP_ND && zwFrame.Command == 0x01)
                    {
                        ...
                        // if (F_AddingSensor)
                        {
                            Func del = delegate
                            {
                                ...
                                int newrows = ultraGrid3.Rows.Count();

                                for (int j = 0; j < rows; j++)
                                {
                                    for (int k = 0; k < newrows; k++)
                                    {
                                        if(ultraGrid1.Rows[j].Cells["Name"].Value.ToString() == ultraGrid3.Rows[k].Cells["Name"].Value.ToString())
                                        {
                                            for(int kk =0; kk < ultraGrid2.Rows.Count; kk++)
                                            {
                                                ...
                                                {
                                                    ultraGrid2.Rows[kk].Cells["DeviceMAC"].Value = ultraGrid3.Rows[k].Cells["MAC"].Value.ToString();
                                                }
                                            }
                                        }
                                    }
